...
const alumniSchema = baseSchema.extend({
  ...
  graduation_year: z.string().refine((val) => {
    ...
    message: "Please enter a valid graduation year between 2000 and current year",
  }),
  ...
  linkedin_url: z.string()
    ...
    .refine((url) => url.includes("linkedin.com"), {
      message: "Please enter a valid LinkedIn URL (should contain linkedin.com)",
    }),
  is_mentorship_available: z.boolean().default(false),
});
...
export default function SignUpPage() {
  ...
  async function onSubmit(data: FormValues) {
    ...
    try {
      ...
      const { data: existingAuthUser, error: authCheckError } = await supabase.auth.getUser();
      if (authCheckError) {
      ...
      if (existingAuthUser?.user) {
        console.log('Found existing auth session, signing out...');
        const { error: signOutError } = await supabase.auth.signOut();
        if (signOutError) {
          console.error('Error signing out:', signOutError);
          throw new Error('Error signing out existing session');
        }
      }
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>
                          Please enter your complete LinkedIn profile URL
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                    ...
                    render={({ field }) => (
                      <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
                        ...
                        <div className="space-y-1 leading-none">
                          ...
                          <FormDescription>
                            This will allow students to request mentorship from you
                          </FormDescription>
                        </div>
                      </FormItem>
                    )}
